...
public class MasterFile extends DirectoryFile {
    ...
    public byte addDF(byte[] AID, byte[] keySettings) {
        ...
        if (searchAID(AID) != (byte) -1)
            ISOException.throwIt(Util.DUPLICATE_ERROR);//AID repetida
        if (numApp == 27)
            ISOException.throwIt((short) 0x91CE);//Num App excede las 28
    ...
    public byte searchAID(byte[] AID) {
        for (byte i = 0; i < indexDF.size; i++) {
            if (javacard.framework.Util.arrayCompare(AID, (short) 0, indexDF.readValue(i), (short) 0, (short) 3) == 0)
                return (i);
        }
    ...
    public void setConfiguration(byte configuration) {
        ...
        if ((configuration & (byte) 0x01) == (byte) 0x01)
            formatEnabled = false;
        else
            formatEnabled = true;
        if ((configuration & (byte) 0x02) == (byte) 0x02)
            randomID = false;
        else
            randomID = true;
    }
    ...
    /**
     * Checks if the key number is 0 since there is only the Master Key in the
     * card level
     */
