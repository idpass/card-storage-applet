...
/**
 * DESfire Card operating system's emulation. This class installs the applet in
 * the card and runs the OS reading APDU's and calling the required functions
 * depending on the INS field
 * 
 * @author WinXp
 */
public class DesfireCard implements VirtualCard {
    ...
    /**
     * Keeps the number of the key that is going to be authenticated during the
     ...
     * EidCard instance needs to be protected so that it can be invoked by
     * subclasses
     */
    protected DesfireCard(short id, RandomData random, Cipher cipher) {
        this.id = id;
            ...
            case Util.CONTINUE:
                switch (commandToContinue) {
                    ...
                    case Util.GET_APPLICATION_IDS:
                        ...
                        break;
                    case Util.READ_DATA:
                        sendBlockResponse(apdu, buffer, dataBuffer, offset, fileSecurityLevel);
                        break;
                    case Util.WRITE_DATA:
                        writeData(apdu, buffer);
                        break;
    ...
    /**
     * PICC and reader device show in an encrypted way that they posses the same
     * key.
     * 
     * @effect Confirms that both entities are permited to do operations on each
     *         other and creates a session key.
     * @note This procedure has two parts. depending on the commandToContinue
     *       status.
     * @note ||KeyNumber||
     */
    private void authenticate(APDU apdu, byte[] buffer) {
        ...
        if (commandToContinue == Util.NO_COMMAND_TO_CONTINUE) {
            ...
            if (!selectedDF.isValidKeyNumber(keyNumberToAuthenticate))
            ...
            if (selectedDF.isMasterFile())
                cipher.init(selectedDF.getMasterKey(), Cipher.MODE_ENCRYPT);
            else
                cipher.init(selectedDF.getKey(keyNumberToAuthenticate), Cipher.MODE_ENCRYPT);
            ...
            //SECCOND MESSAGE 
            if ((byte) (buffer[ISO7816.OFFSET_LC]) != 16)
                ISOException.throwIt(Util.LENGTH_ERROR);
            commandToContinue = Util.NO_COMMAND_TO_CONTINUE;
            ...
            encryptedRndArndB =
                                Util.subByteArray(buffer, (byte) ISO7816.OFFSET_CDATA,
                                                  (byte) (ISO7816.OFFSET_CDATA + 16));
            if (selectedDF.isMasterFile())
                cipher.init(selectedDF.getMasterKey(), Cipher.MODE_DECRYPT);
            else
            ...
            //RndB is checked
            if (javacard.framework.Util.arrayCompare(rndB, (short) 0, randomNumberToAuthenticate, (short) 0,
                                                     (short) rndB.length) != 0) {
     ...
     * 
     * @note ||Ciphered Key Settings|| 8/16
     */
    private void changeKeySettings(APDU apdu, byte[] buffer) {
        ...
        if (!selectedDF.hasKeySettingsChangeAllowed(authenticated))
     ...
     * 
     * @note ||Key number | Ciphered Key Data|| 1 24-40
     */
    private void changeKey(APDU apdu, byte[] buffer) {
        ...
        if ((selectedDF.isMasterFile() == true) && (keyN != 0))
            ISOException.throwIt(Util.PARAMETER_ERROR);
        if ((selectedDF.isMasterFile() == false) && (keyN >= 28))
            ISOException.throwIt(Util.PARAMETER_ERROR);
        if (selectedDF.hasChangeAccess(authenticated, keyN) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
     ...
     * 
     * @note || AID | KeySettings1 | KeySettings2 | ISOFileID* | DF_FILE* || 3 1
     *       1 2 1-16
     */
    private void createApplication(APDU apdu, byte[] buffer) {
        ...
        if (masterFile.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        if (masterFile.getIndexDF().hasWriteAccess((byte) 0) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);//CREO QUE SOBRA
        byte[] AID = {
                buffer[ISO7816.OFFSET_CDATA], buffer[ISO7816.OFFSET_CDATA + 1], buffer[ISO7816.OFFSET_CDATA + 2]
        };
        byte[] keySettings = {
                buffer[ISO7816.OFFSET_CDATA + 3], buffer[ISO7816.OFFSET_CDATA + 4]
        };
     ...
     * 
     * @effect If the application that is going to be removed is the currently
     *         selected the PICC level is set
     * @note || AID || 3
     */
    private void deleteApplication(APDU apdu, byte[] buffer) {

        receiveAPDU(apdu, buffer);
        if (((byte) (buffer[ISO7816.OFFSET_LC]) != 3))
            ISOException.throwIt(Util.LENGTH_ERROR);
        byte[] AID = {
                buffer[ISO7816.OFFSET_CDATA], buffer[ISO7816.OFFSET_CDATA + 1], buffer[ISO7816.OFFSET_CDATA + 2]
        };
        if (masterFile.searchAID(AID) == -1)
            ISOException.throwIt(Util.APPLICATION_NOT_FOUND);
        if (masterFile.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);

        //If the application that is going to be removed is the currently selected the PICC level is set
        if (selectedDF.getFileID() == masterFile.searchAID(AID))
            selectedDF = masterFile;
    ...
    private void getApplicationIDs(APDU apdu, byte[] buffer) {
        receiveAPDU(apdu, buffer);
        if ((byte) buffer[ISO7816.OFFSET_LC] != 0)
            ISOException.throwIt(Util.LENGTH_ERROR);
        byte[] response;
        byte numApp = (byte) (masterFile.numApp - 1);//-1 because the IndexFile won't be included
        if (commandToContinue == Util.NO_COMMAND_TO_CONTINUE) {
            if (masterFile.hasGetRights(authenticated) == false)
                ISOException.throwIt(Util.PERMISSION_DENIED);
            if (numApp == 0)
                ISOException.throwIt(Util.OPERATION_OK);
            //			if(numApp==1){
            ...
            if (numApp > 19) {
                ...
                response = new byte[(byte) (numApp * 3)];
    ...
    /**
     * Get information on the PICC and application master key settings. In
     * addition it returns the maximum number of keys which are configured for
    ...
    private void getKeySettings(APDU apdu, byte[] buffer) {
        ...
        if ((byte) buffer[ISO7816.OFFSET_LC] != 0)
            ISOException.throwIt(Util.LENGTH_ERROR);
        if (!selectedDF.hasGetRights(authenticated))
            ISOException.throwIt(Util.PERMISSION_DENIED);
    ...
    private void selectApplication(APDU apdu, byte[] buffer) {
        ...
        if (javacard.framework.Util.arrayCompare(AID, (short) 0, Util.masterFileAID, (short) 0, (short) AID.length) == 0) {
            ...
            if (i != (byte) -1)
                selectedDF = masterFile.arrayDF[masterFile.searchAID(AID)];
            else
                ISOException.throwIt(Util.APPLICATION_NOT_FOUND);
        }
        authenticated = Util.NO_KEY_AUTHENTICATED;
     ...
     * @effect All application are deleted and all files within them. The PICC
     *         Master Keyand the PICC Master Key settings keep their currently
     *         set values
     */
    private void formatPICC(APDU apdu, byte[] buffer) {
        if (selectedDF.isMasterFile() == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        receiveAPDU(apdu, buffer);
        if ((byte) buffer[ISO7816.OFFSET_LC] != 0)
            ISOException.throwIt(Util.LENGTH_ERROR);
        if (masterFile.isFormatEnabled() == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        if (authenticated != 0)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        masterFile.format();
     ...
     * 
     * @exception Master
     *                key authentication on card level needs to be performed
     *                elsewise throw PERMISSION_DENIED
    ...
    private void getFileIDs(APDU apdu, byte[] buffer) {
        if (selectedDF.isMasterFile() == true)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        receiveAPDU(apdu, buffer);
        if ((byte) buffer[ISO7816.OFFSET_LC] != 0)
            ISOException.throwIt(Util.LENGTH_ERROR);
     ...
     * 
     * @exception Throw
     *                PERMISION_DENIED if card level is selected or the
     *                application's configuration doesn't allow manage for the
     *                current authentication state.
     * @note The MSB in the 3 bytes values is not readed.
     * @note || File Number | ISO7816 FileID* | CommunicationSettings |
     *       AccessRights | FileSize(3) || 1 2 1 2 3
     */
    private void createStdDataFile(APDU apdu, byte[] buffer) {
        ...
        if (((byte) buffer[ISO7816.OFFSET_LC] != 7) && ((byte) buffer[ISO7816.OFFSET_LC] != 9))
            ISOException.throwIt(Util.LENGTH_ERROR);
        if (selectedDF.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);

        byte fileID = (byte) buffer[ISO7816.OFFSET_CDATA];
        if (selectedDF.isValidFileNumber(fileID) == true)
            ISOException.throwIt(Util.DUPLICATE_ERROR);
        ...
        if (buffer[ISO7816.OFFSET_LC] == 9) {
            ...
            accessPermissions = new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 5], (byte) buffer[ISO7816.OFFSET_CDATA + 4]
            };
            size = new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 7], (byte) buffer[ISO7816.OFFSET_CDATA + 6]
            };
        } else {
            communicationSettings = (byte) buffer[ISO7816.OFFSET_CDATA + 1];
            accessPermissions = new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 3], (byte) buffer[ISO7816.OFFSET_CDATA + 2]
            };
            size = new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 5], (byte) buffer[ISO7816.OFFSET_CDATA + 4]
            };
        ...
        if (sizeS > (short) JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_PERSISTENT))
            ISOException.throwIt(Util.OUT_OF_EEPROM_ERROR);
        selectedFile =
                       new StandartFile(fileID, masterFile.arrayDF[selectedDF.getFileID()], communicationSettings,
                                        accessPermissions, sizeS);
    ...
    /**
     * Creates files for the storage of plain unformatted user data within an
     * existing application on the PICC, additionally supporting the feature of
    ...
    private void createBackupDataFile(APDU apdu, byte[] buffer) {
        ...
        if (sizeS > (short) JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_PERSISTENT))
            ISOException.throwIt(Util.OUT_OF_EEPROM_ERROR);
        selectedFile =
                       new BackupFile(fileID, masterFile.arrayDF[selectedDF.getFileID()], communicationSettings,
                                      accessPermissions, sizeS);
        selectedDF = masterFile.arrayDF[selectedDF.getFileID()];
        ISOException.throwIt(Util.OPERATION_OK);
    }
     ...
     * 
     * @note || FileN | CommunicationSetting | AccessRights | LowerLimit(4) |
     *       UpperLimit(4) | Value(4) | LimitedCreditEnabled || 1 1 2 4 4 4 1
     */
    private void createValueFile(APDU apdu, byte[] buffer) {
        if (selectedDF.isMasterFile() == true)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        receiveAPDU(apdu, buffer);
        if ((byte) buffer[ISO7816.OFFSET_LC] != 17)
            ISOException.throwIt(Util.LENGTH_ERROR);
        byte fileID = (byte) buffer[ISO7816.OFFSET_CDATA];
        if (selectedDF.isValidFileNumber(fileID) == true)
            ISOException.throwIt(Util.DUPLICATE_ERROR);
        ...
        Value upperLimit =
                           new Value(new byte[] {
                                   (byte) buffer[ISO7816.OFFSET_CDATA + 11], (byte) buffer[ISO7816.OFFSET_CDATA + 10],
                                   (byte) buffer[ISO7816.OFFSET_CDATA + 9], (byte) buffer[ISO7816.OFFSET_CDATA + 8]
                           });
        Value value =
                      new Value(new byte[] {
                              (byte) buffer[ISO7816.OFFSET_CDATA + 15], (byte) buffer[ISO7816.OFFSET_CDATA + 14],
                              (byte) buffer[ISO7816.OFFSET_CDATA + 13], (byte) buffer[ISO7816.OFFSET_CDATA + 12]
                      });
        if (upperLimit.compareTo(lowerLimit) != 1)
            ISOException.throwIt(Util.BOUNDARY_ERROR);
        if (upperLimit.compareTo(value) != 1)
            ISOException.throwIt(Util.BOUNDARY_ERROR);
        if (value.compareTo(lowerLimit) != 1)
            ISOException.throwIt(Util.BOUNDARY_ERROR);
        byte limitedCreditEnabled = (byte) buffer[ISO7816.OFFSET_CDATA + 16];
        if ((short) (30) > (short) JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_PERSISTENT))
            ISOException.throwIt(Util.OUT_OF_EEPROM_ERROR);
        if (selectedDF.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        selectedFile =
                       new ValueRecord(fileID, masterFile.arrayDF[selectedDF.getFileID()], communicationSettings,
                                       accessPermissions, lowerLimit, upperLimit, value, limitedCreditEnabled);
        selectedDF = masterFile.arrayDF[selectedDF.getFileID()];
        ISOException.throwIt(Util.OPERATION_OK);
    }

    /**
     * Creates files for multiple storage of structural similar data within an
     * existing application on the PICC.
     * 
     ...
     * @note || File Number | ISO7816 FileID* | CommunicationSettings |
     *       AccessRights | RecordSize | MaxNumRecords || 1 2 1 2 3 3
    ...
    private void createLinearRecordFile(APDU apdu, byte[] buffer) {
        ...
        if (buffer[ISO7816.OFFSET_LC] == 10) {
            ...
            recordSize = Util.byteArrayToShort(new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 5], (byte) buffer[ISO7816.OFFSET_CDATA + 4]
            });
            maxRecordNum = Util.byteArrayToShort(new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 8], (byte) buffer[ISO7816.OFFSET_CDATA + 7]
            });
            ...
            maxRecordNum = Util.byteArrayToShort(new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 10], (byte) buffer[ISO7816.OFFSET_CDATA + 9]
            });
        }

        if ((short) (recordSize * maxRecordNum) > (short) JCSystem.getAvailableMemory(JCSystem.MEMORY_TYPE_PERSISTENT))
            ISOException.throwIt(Util.OUT_OF_EEPROM_ERROR);
        if (selectedDF.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        selectedFile =
                       new LinearRecord(fileID, masterFile.arrayDF[selectedDF.getFileID()], communicationSettings,
                                        accessPermissions, recordSize, maxRecordNum);
        selectedDF = masterFile.arrayDF[selectedDF.getFileID()];
        ISOException.throwIt(Util.OPERATION_OK);
    }
     ...
     * existing application on the PICC.
     * 
     * @note Once the file is filled completely with data records, the oldest
     *       record is overwritten with the latest written one.
     * @note || File Number | ISO7816 FileID | CommunicationSettings |
     *       AccessRights | RecordSize(3) | MaxNumRecords(3) ||
     * @note The MSB in the 3 bits values is not readed.
     */
    private void createCyclicRecordFile(APDU apdu, byte[] buffer) {
        ...
        if (selectedDF.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        selectedFile =
                       new CyclicRecord(fileID, masterFile.arrayDF[selectedDF.getFileID()], communicationSettings,
                                        accessPermissions, recordSize, maxRecordNum);
        selectedDF = masterFile.arrayDF[selectedDF.getFileID()];
        ISOException.throwIt(Util.OPERATION_OK);
    }
    ...
    private void deleteFile(APDU apdu, byte[] buffer) {
        ...
        if ((byte) buffer[ISO7816.OFFSET_LC] != 1)
            ISOException.throwIt(Util.LENGTH_ERROR);
        byte fileID = (byte) buffer[ISO7816.OFFSET_CDATA];
        if (selectedDF.isValidFileNumber(fileID) == false)
            ISOException.throwIt(Util.FILE_NOT_FOUND);
        if (selectedDF.hasManageRights(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
     ...
     * 
     * @note The MSB in the 3 bits values is not readed
     * @note This method just send the first (or only) message, if the readed
     *       data doesn't fit in one APDU the system will call the sendBlockData
     * @note || FileNumber | Offset | Length || 1 3 3
     */
    private void readData(APDU apdu, byte[] buffer) {
        if (selectedDF.isMasterFile() == true)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        ...
        if (((StandartFile) selectedFile).hasReadAccess(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        offset = Util.byteArrayToShort(new byte[] {
                (byte) buffer[ISO7816.OFFSET_CDATA + 2], (byte) buffer[ISO7816.OFFSET_CDATA + 1]
        });
        bytesLeft = Util.byteArrayToShort(new byte[] {
                (byte) buffer[ISO7816.OFFSET_CDATA + 5], (byte) buffer[ISO7816.OFFSET_CDATA + 4]
        });
        if (bytesLeft == 0)
            bytesLeft = selectedFile.getSize();

        //Read data
        dataBuffer = ((StandartFile) selectedFile).readArray(offset, bytesLeft, (short) 0);
        commandToContinue = Util.READ_DATA;

        sendBlockResponse(apdu, buffer, dataBuffer, (short) 0, selectedFile.getCommunicationSettings());
     ...
     * 
     * @note The MSB in the 3 bits values is not readed
     * @note If the data doesn't fit in one message (52 bytes) the sender will
     *       split it in more messages (59 bytes) so this command may have more
     *       than one execution in row.
     * @note || File No | Offset | Lenght | Data || 1 3 3 1-52
     */
    private void writeData(APDU apdu, byte[] buffer) {
        if (selectedDF.isMasterFile() == true)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        ...
        if (commandToContinue == Util.NO_COMMAND_TO_CONTINUE) {
            ...
            if (selectedFile.hasWriteAccess(authenticated) == false)
                ISOException.throwIt(Util.PERMISSION_DENIED);
            ...
            dataBuffer =
                         Util.subByteArray(buffer, (short) (ISO7816.OFFSET_CDATA + 7), (short) (ISO7816.OFFSET_CDATA
                                                                                                + 7 + readed - 1));
            ...
            bytesLeft = (short) (bytesLeft - (short) readed);
            dataBuffer =
                         Util.concatByteArray(dataBuffer, Util.subByteArray(buffer, ISO7816.OFFSET_CDATA,
                                                                            (short) (ISO7816.OFFSET_CDATA + readed)));
        }
    ...
    private void credit(APDU apdu, byte[] buffer) {
        ...
        if (((ValueRecord) selectedFile).hasWriteAccess(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        ((ValueRecord) selectedFile).addCredit(new Value(new byte[] {
                (byte) buffer[ISO7816.OFFSET_CDATA + 4], (byte) buffer[ISO7816.OFFSET_CDATA + 3],
                (byte) buffer[ISO7816.OFFSET_CDATA + 2], (byte) buffer[ISO7816.OFFSET_CDATA + 1]
        }));
        ISOException.throwIt(Util.OPERATION_OK);
    }
    ...
    private void debit(APDU apdu, byte[] buffer) {
        ...
        if (((ValueRecord) selectedFile).hasWriteAccess(authenticated) == false)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        ((ValueRecord) selectedFile).decDebit(new Value(new byte[] {
                (byte) buffer[ISO7816.OFFSET_CDATA + 4], (byte) buffer[ISO7816.OFFSET_CDATA + 3],
                (byte) buffer[ISO7816.OFFSET_CDATA + 2], (byte) buffer[ISO7816.OFFSET_CDATA + 1]
        }));
        ISOException.throwIt(Util.OPERATION_OK);
     ...
     *       split it in more messages (59 bytes) so this command may have more
     *       than one execution in row.
     * @note || FileN | Offset | Length | Data || 1 3 3 1-52
     */
    ...
    private void writeRecord(APDU apdu, byte[] buffer) {
        ...
        if (commandToContinue == Util.NO_COMMAND_TO_CONTINUE) {
            ...
            if (bytesLeft <= 52) {
                if (selectedFile instanceof LinearRecord)
                    ((LinearRecord) selectedFile).writeRecord(dataBuffer, offset);
                if (selectedFile instanceof CyclicRecord)
                    ((CyclicRecord) selectedFile).writeRecord(dataBuffer, offset);
                dataBuffer = null;
                offset = 0;
                bytesLeft = 0;
                ISOException.throwIt(Util.OPERATION_OK);
     ...
     *       message (59 bytes) the data field is splitted. If more thata will
     *       be sent the PICC informs with the SW: 0xAF
     * @note || FileN | Offset | Length || 1 3 3 Offset. Position of the newest
     *       record to read starting from the end Length. Number of records to
     *       read
     */
    ...
    private void readRecords(APDU apdu, byte[] buffer) {
        if (selectedDF.isMasterFile() == true)
            ISOException.throwIt(Util.PERMISSION_DENIED);
        receiveAPDU(apdu, buffer);
        if ((byte) buffer[ISO7816.OFFSET_LC] != 7)
            ISOException.throwIt(Util.LENGTH_ERROR);
        byte[] out = null;
        if (commandToContinue == Util.NO_COMMAND_TO_CONTINUE) {
            byte fileID = buffer[ISO7816.OFFSET_CDATA];
            ...
            short length = Util.byteArrayToShort(new byte[] {
                    (byte) buffer[ISO7816.OFFSET_CDATA + 5], (byte) buffer[ISO7816.OFFSET_CDATA + 4]
            });
            if (selectedFile instanceof LinearRecord) {
                selectedFile = (LinearRecord) selectedDF.getFile(buffer[ISO7816.OFFSET_CDATA]);
                ...
                offset =
                         (short) (((LinearRecord) selectedFile).getCurrentSize() - offset
                                  * ((LinearRecord) selectedFile).recordSize - bytesLeft);//offset respecto al inicio
                if (bytesLeft <= 59) {
            ...
            if (selectedFile instanceof CyclicRecord) {
                ...
                offset =
                         (short) (((CyclicRecord) selectedFile).getNextToWrite() - offset
                                  * ((CyclicRecord) selectedFile).recordSize - bytesLeft);//offset respecto al inicio
                if (bytesLeft <= 59) {
     ...
     * 
     * @return The message encrypted in the following way: - The CRC16 is
     *         calculated - The whole array is padded - Everything is encyphered
     ...
     * 
     * @return The message decrypted in the following way: - Everything is
     *         decyphered - The padding is taken out - The CRC16 is calculated
     *         and compared with the received
     * @note For the moment it only uses 3DES
     */
    ...
    private void receiveAPDU(APDU apdu, byte[] buffer) {
        ...
        byte[] cData =
                       getCData(Util.subByteArray(buffer, (byte) ISO7816.OFFSET_CDATA,
                                                  (byte) (ISO7816.OFFSET_CDATA + buffer[ISO7816.OFFSET_LC] - 1)));
    ...
    private void sendResponse(APDU apdu, byte[] buffer, byte[] response, short status, byte securityLevel) {
        ...
        try {
            ...
            if (response.length == 0)
                ISOException.throwIt((short) 0x917E);//AUX
            //			this.securityLevel=Util.PLAIN_COMMUNICATION;
    ...
    /**
     * This is needed for the authentication because the last message should be
     * sended encrypted with the old session key and afterwards the session key
     * should change
     */
    ...
    /**
     * Initialize the applet when it is selected, select always has to happen
     * after a reset
     */
     ...
     * security mechanism
     * 
     * @note If the key number to change is different from the key used for
     *       authentication, it is needed to prove that the other key is also
     *       known so the PCD has to: - bit-wise XOR both new and old key -
     ...
     *       append at the end the CRC16 calculated over the new key - Do the
     *       paddingto reach an adequate frame size - Encipher using he current
     *       session key - The blocks are chained in CRC send mode.
     */
    public byte[] decryptEncipheredKeyData(byte[] encipheredData, byte keyN) {
        ...
        if (keyN == authenticated) {
            ...
            if (Util.byteArrayCompare(XORCrc, receivedXORCrc) == false) {
                //We check if there was no padding
                receivedXORCrc =
                                 Util.subByteArray(unpaddedData, (byte) (unpaddedData.length - 4),
                                                   (byte) (unpaddedData.length - 3));
            ...
            if (Util.byteArrayCompare(newKeyCrc, receivedNewKeyCrc) == false) {
                //We check if there was no padding
                receivedNewKeyCrc =
                                    Util.subByteArray(unpaddedData, (byte) (unpaddedData.length - 2),
                                                      (byte) (unpaddedData.length - 1));
    ...
    /**
     * It is called for the continuous sending of parts of a message. Everytime
     * it is called it sends a block of MAX_BLOCK bytes since the position
     * pointed by the given offset
     */
    private void sendBlockResponse(APDU apdu, byte[] buffer, byte[] data, short offset, byte securityLevel) {
        if ((short) (offset + Util.MAX_DATA_SIZE) < data.length) {
            this.offset = (short) (offset + Util.MAX_DATA_SIZE);
            sendResponse(apdu, buffer, Util.subByteArray(data, offset, (short) (offset + Util.MAX_DATA_SIZE - 1)),
                         Util.CONTINUE, securityLevel);
        } else {
            //			ISOException.throwIt((short)securityLevel);
            sendResponse(apdu, buffer, Util.subByteArray(data, offset, (short) (data.length - 1)), Util.CONTINUE,
                         securityLevel);
            halfClear();
        }
